...
def sync_metrics_update(context):
    ...
    st.session_state.form_score = latest_metrics.get("form_score", 100)
    ...
    # only run ongoing form check if no audio is already pending
    if not st.session_state.get("audio_to_play") and st.session_state.get("voice_pipeline"):
        result = st.session_state.voice_pipeline.process_event(
