...
class VoicePipeline:
    ...
    def process_event(self, event, exercise, metrics):
        ...
        is_major_issue = event in ["login", "workout_started", "set_completed", "workout_completed"]
        ...
        (function() {{
            function playAudio() {{
                ...
                audio.play().then(function() {{
                    ...
                    console.warn('[AI Coach] Autoplay blocked, retrying on next interaction:', e);
                    // fallback: play on next user click anywhere
                    var handler = function() {{
                        audio.play().catch(function(){{}});
                        document.removeEventListener('click', handler);
                        document.removeEventListener('keydown', handler);
                    }};
                    window.parent.document.addEventListener('click', handler, {{once: true}});
                    window.parent.document.addEventListener('keydown', handler, {{once: true}});
                }});
            }}
            playAudio();
        }})();
